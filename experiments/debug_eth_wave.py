...
def debug_wave_eth():
    ...
    # ML Filter
    model_file = 'wizard_wave_ml_model.pkl'
    if os.path.exists(model_file):
        loaded = joblib.load(model_file)
        model = loaded['model']
        thresh = 0.40 # Threshold used in comparison
        ...
        if sig_mask.any():
            ...
            for idx, row in df[sig_mask].iterrows():
                df.at[idx, 'ml_prob'] = probs[prob_idx]
    ...
    print(f"| Date | Action | Price | Prob | SL | PnL | Equity |")
    ...
    for idx, row in df.iterrows():
        ...
        if position != 0:
            ...
            if position == 1:
                if sl > 0 and row['low'] <= sl:
                    pnl_val = (sl - entry_price) * (equity*exposure/entry_price)
                    closed = True
                elif tp > 0 and row['high'] >= tp:
                    pnl_val = (tp - entry_price) * (equity*exposure/entry_price)
                    closed = True
                elif 'SHORT' in sig:
                    pnl_val = (price - entry_price) * (equity*exposure/entry_price)
                    closed = True
            elif position == -1:
                if sl > 0 and row['high'] >= sl:
                    pnl_val = (entry_price - sl) * (equity*exposure/entry_price)
                    closed = True
                elif tp > 0 and row['low'] <= tp:
                    pnl_val = (entry_price - tp) * (equity*exposure/entry_price)
                    closed = True
                elif 'LONG' in sig:
                    pnl_val = (entry_price - price) * (equity*exposure/entry_price)
                    closed = True
        ...
        if position == 0:
            if 'LONG' in sig:
                position = 1
                entry_price = price
                print(f"| {idx} | LONG | {price:.2f} | {row.get('ml_prob', 0):.2f} | {sl:.2f} | - | - |")
            elif 'SHORT' in sig:
                position = -1
                entry_price = price
                print(f"| {idx} | SHORT | {price:.2f} | {row.get('ml_prob', 0):.2f} | {sl:.2f} | - | - |")
