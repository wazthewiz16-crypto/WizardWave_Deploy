...
try:
    with open(os.path.join(root_path, 'config', 'discord_config.json'), 'r') as f:
        DISCORD_CONFIG = json.load(f)
        WEBHOOK_URL = DISCORD_CONFIG.get('webhook_url')
except:
    WEBHOOK_URL = None

PROCESSED_FILE = os.path.join(root_path, 'data', 'processed_signals.json')
...
def run_analysis_cycle():
    ...
    for name, conf in models_config.items():
        try:
            model_path = os.path.join(root_path, conf['model_file'])
